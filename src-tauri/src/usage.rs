...
pub(crate) async fn fetch_usage(
    access_token: &str,
    account_id: &str,
) -> Result<FetchedUsage, String> {
    let usage_urls = resolve_usage_urls();

    let client = reqwest::Client::builder()
        .user_agent("codexs/0.3")
...
fn map_usage_payload(payload: UsageApiResponse) -> Result<FetchedUsage, String> {
    ...
    let five_hour = pick_nearest_window(&windows, 5 * 60 * 60)
        .map(to_usage_window)
        .ok_or_else(|| "用量接口未返回 5h 窗口".to_string())?;
    let one_week = pick_nearest_window(&windows, 7 * 24 * 60 * 60)
        .map(to_usage_window)
        .ok_or_else(|| "用量接口未返回 1week 窗口".to_string())?;
